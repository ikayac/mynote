...
def calculate_log_likelihood_advanced(data, model_name, params):
    """対数尤度の計算（高度版・修正版）"""
    log_likelihood = 0
            ...
            if x >= truncation_point:
                ...
                for j in range(len(mus)):
                    ...
                    if sigmas[j] > 0 and z < 10:
                        ...
                        if norm_factor > 1e-10:
                            component_likelihood += weights[j] * normal_pdf(x, mus[j], sigmas[j]) / norm_factor
...
def main():
    ...
    for name, model in models.items():
        log_likelihood = calculate_log_likelihood_advanced(data, name, model['params'])
    ...
    if best_model['type'] == 'mixture_truncated_normal':
        ...
        params = best_model['params']
        print(f"第1成分: μ={params['mus'][0]:.2f}, σ={params['sigmas'][0]:.2f}, 重み={params['weights'][0]:.3f}")
        print(f"第2成分: μ={params['mus'][1]:.2f}, σ={params['mus'][1]:.2f}, 重み={params['weights'][1]:.3f}")
